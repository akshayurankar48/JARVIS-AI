...
class Install_Plugin implements Action_Interface {
	...
	public function execute( array $params ): array {
		...
		if ( empty( $slug ) ) {
			return array(
				'success' => false,
				'data'    => null,
				'message' => __( 'Plugin slug or search term is required.', 'jarvis-ai' ),
			);
		}
		...
		if ( $existing ) {
			return array(
				...
				'message' => sprintf(
					...
					__( 'Plugin "%s" is already installed. Use activate_plugin to activate it.', 'jarvis-ai' ),
					$existing['name']
				),
			);
		}
			...
			if ( is_wp_error( $search_result ) ) {
				return array(
					...
					'message' => sprintf(
						...
						__( 'Could not find plugin "%1$s" on WordPress.org. Searched by slug and keyword. %2$s', 'jarvis-ai' ),
						$slug,
		...
		if ( $existing ) {
			return array(
				...
				'message' => sprintf(
					...
					__( 'Plugin "%1$s" (matched from "%2$s") is already installed. Use activate_plugin to activate it.', 'jarvis-ai' ),
					$existing['name'],
					$slug
				),
			);
	...
	private function search_and_resolve( $query ) {
		...
		if ( empty( $plugins ) ) {
			return new \WP_Error(
				'no_results',
				__( 'No plugins found matching that search term.', 'jarvis-ai' )
			);
		}
		...
		if ( empty( $best_slug ) ) {
			return new \WP_Error(
				'no_slug',
				__( 'Search returned results but could not determine plugin slug.', 'jarvis-ai' )
			);
		}
	...
	private function install_from_api( $api, $original_slug ) {
		...
		if ( ! $result ) {
			return array(
				...
				'message' => sprintf(
					...
					__( 'Failed to install plugin "%s". Check filesystem permissions.', 'jarvis-ai' ),
		...
		$message       = sprintf(
			...
			__( 'Installed "%1$s" (v%2$s) successfully. Use activate_plugin to activate it.', 'jarvis-ai' ),
			$name,
